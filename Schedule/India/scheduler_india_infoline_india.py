...
import india_infoline as loader
import schedule
...
# Global variables
market_hours_init = 0
lookup_value = "india_infoline"  # search website name
...
def start_process():
    ...
    try:
        ...
        print("***********Initiate Process - india_infoline**************")
        database_log.process_log("India - india_infoline : start_process", "Initiate Process")

        while True:
            # Checks whether a scheduled task - is pending to run or not
            scheduled_sleeping_seconds = app_config.india_market_scheduled_task_sleeping
            loader.start_load_process(lookup_value)
            database_log.process_log("India - india_infoline : start_process", "Re-Run Process")

            print("Last run was successful for India - india_infoline, next run in {} seconds.".format(scheduled_sleeping_seconds))
            time.sleep(scheduled_sleeping_seconds)

    except Exception as error:
        database_log.error_log("India - india_infoline : start_process", error)
