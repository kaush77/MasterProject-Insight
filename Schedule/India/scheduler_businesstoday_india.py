...
import businesstoday as loader
import schedule
...
# Global variables
market_hours_init = 0
lookup_value = "businesstoday"  # search website name
...
def start_process():
    ...
    try:
        ...
        database_log.process_log("India - business_today : start_process", "Initiate Process")

        while True:
            # Checks whether a scheduled task - is pending to run or not
            scheduled_sleeping_seconds = app_config.india_market_scheduled_task_sleeping
            loader.start_load_process(lookup_value)
            database_log.process_log("India - business_today : start_process", "Re-Run Process")

            print("Last run was successful for India - business_today, next run in {} seconds.".format(scheduled_sleeping_seconds))
