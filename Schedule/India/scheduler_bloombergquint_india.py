...
import bloombergquint as loader
import schedule
...
# Global variables
market_hours_init = 0
lookup_value = "bloombergquint"  # search website name
...
def start_process():
    ...
    try:
        ...
        print("***********Initiate Process - bloombergquint**************")
        database_log.process_log("India - bloombergquint : start_process", "Initiate Process")
        # start_scheduler()
        ...
        while True:
            ...
            loader.start_load_process(lookup_value)
            database_log.process_log("India - bloombergquint : start_process", "Re-Run Process")

            print("Last run was successful for India - bloombergquint, next run in {} seconds.".format(scheduled_sleeping_seconds))
            time.sleep(scheduled_sleeping_seconds)

    except Exception as error:
        database_log.error_log("India - bloombergquint : start_process", error)
