...
import money_control as loader
import schedule
...
# Global variables
market_hours_init = 0
lookup_value = "money_control"  # search website name
...
def start_process():
    ...
    try:
        ...
        print("***********Initiate Process - money_control**************")
        database_log.process_log("India - money_control : start_process", "Initiate Process")

        while True:
            # Checks whether a scheduled task - is pending to run or not
            scheduled_sleeping_seconds = app_config.india_market_scheduled_task_sleeping
            loader.start_load_process(lookup_value)
            database_log.process_log("India - money_control : start_process", "Re-Run Process")

            print("Last run was successful for India - money_control, next run in {} seconds.".format(scheduled_sleeping_seconds))
            time.sleep(scheduled_sleeping_seconds)

    except Exception as error:
        database_log.error_log("India - money_control : start_process", error)
