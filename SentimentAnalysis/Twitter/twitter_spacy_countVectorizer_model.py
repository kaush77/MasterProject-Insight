...
def save_train_model(model, file_name):
    ...
    file_path = "../TrainModel/Twitter/Spacy_CountVectorizer/" + file_name
...
def vectorization_pipeline(classifier):
    # tokenize words needs to be lemmatized and filtered for pronouns, stopwords and punctuations
    vectorization = CountVectorizer(tokenizer=tokenizer, ngram_range=(1, 1))
...
def train_count_vectorizor_model(X_train, X_test, y_train, y_test,classifier):
...
if __name__ == "__main__":
    ...
    # save model result
    model_result_df.to_csv("../TrainModel/Twitter/Spacy_CountVectorizer/CountVectorizerModelResult.csv")
