...
def read_information(df_web_config):
    ...
    try:
        ...
        for row in df_web_config.itertuples(index=False):
            ...
            # http response 200 means OK status
            if resp.status_code == 200:

                # parser
                soup = BeautifulSoup(resp.text, 'html.parser')

                website = 'economic_times'
                ...
                for level_1 in soup.findAll("div", {"class": "tabdata"}):
                    for level_2 in level_1.findAll("div", {"class": "eachStory"}):
                        for level_3 in level_2.findAll("h3"):
                            for level_4 in level_3.findAll("a"):
                                news_link = level_4['href']
                                header = level_4.text

                        for level_3 in level_2.findAll("time", {"class": "date-format"}):
                ...
                database_log.error_log("India - Loader - economic_times : read_information", resp.status_code)
        ...
        if df_news_data.empty is not True:
            ...
            database_log.error_log("India - Loader - economic_times : read_information", "no record found")

    except Exception as error:
        database_log.error_log("India - Loader - economic_times : read_information", error)
...
if __name__ == '__main__':
    start_load_process('economic_times')
