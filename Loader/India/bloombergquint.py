    ...
    try:
        ...
        for row in df_web_config.itertuples(index=False):
            ...
            if resp.status_code == 200:
                ...
                for level_1 in soup.findAll("section", {"class": "topic-page container"}):
                    for level_2 in level_1.findAll("li", {"class": "topic-page__item"}):
                        for level_3 in level_2.findAll("h3", {"class": "topic-page__item__headline"}):
                            for level_4 in level_3.findAll("a"):
                                news_link = level_4['href']
                                header = level_4.text.strip()

                        for level_3 in level_2.findAll("h4", {"class": "topic-page__item__summary"}):
                            sub_header = level_3.text.strip()

                        for level_3 in level_2.findAll("div", {"class": "topic-page__item__timestamp"}):
                            for level_4 in level_3.findAll("time", {"class": "updated-at humanized"}):
                                updated_at = level_4

                            for level_4 in level_2.findAll("time", {"class": "published-at humanized"}):
                                timestamp = level_4.text.strip()
                ...
                database_log.error_log("India - Loader - bloombergquint : read_information", resp.status_code)
        ...
        if df_news_data.empty is not True:
            ...
            database_log.error_log("India - Loader - bloombergquint : read_information", "no record found")

    except Exception as error:
        database_log.error_log("India - Loader - bloombergquint : read_information", error)
...
if __name__ == '__main__':
    start_load_process('bloombergquint')
