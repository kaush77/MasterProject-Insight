    ...
    try:
        ...
        for row in df_web_config.itertuples(index=False):
            ...
            if resp.status_code == 200:
                ...
                for level_1 in soup.findAll("ul"):
                    for level_2 in level_1.findAll("li", {"class": "clearfix"}):
                        for level_3 in level_2.findAll("h2"):
                            for level_4 in level_3.findAll("a"):
                                news_link = level_4['href']
                                header = level_4.text

                        for level_3 in level_2.findAll("span"):
                ...
                database_log.error_log("India - Loader - money_control : read_information", resp.status_code)
        ...
        if df_news_data.empty is not True:
            ...
            database_log.error_log("India - Loader - money_control : read_information", "no record found")

    except Exception as error:
        database_log.error_log("India - Loader - money_control : read_information", error)
...
if __name__ == '__main__':
    start_load_process('money_control')
